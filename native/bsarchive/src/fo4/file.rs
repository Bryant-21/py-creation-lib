...
use directxtex::{
    CP_FLAGS, DDS_FLAGS, DXGI_FORMAT, FORMAT_TYPE, ScratchImage, TEX_DIMENSION, TEX_MISC_FLAG,
    TexMetadata,
};
use std::{borrow::Cow, error, io::Write};
...
///
/// ```rust
/// use bsarchive_native::fo4::{CompressionFormat, FileWriteOptions, Format};
///
...
impl<'bytes> File<'bytes> {
    ...
    fn normalized_xbox_dds<'this>(bytes: &'this [u8]) -> Option<Cow<'this, [u8]>> {
        const DDS_MAGIC_LEN: usize = 4;
        const DDS_PIXELFORMAT_FOURCC_OFFSET: usize = 84;
        const DDS_HEADER_DXT10_MISC_FLAGS2_OFFSET: usize = 144;
        const DDS_HEADER_WITH_DXT10_LEN: usize = 148;
        const XBOX_EXTENSION_LEN: usize = 16;

        if bytes.len() < DDS_HEADER_WITH_DXT10_LEN + XBOX_EXTENSION_LEN
            || &bytes[..DDS_MAGIC_LEN] != b"DDS "
            || &bytes[DDS_PIXELFORMAT_FOURCC_OFFSET..DDS_PIXELFORMAT_FOURCC_OFFSET + 4] != b"XBOX"
        {
            return None;
        }

        let mut normalized = bytes.to_vec();
        normalized[DDS_PIXELFORMAT_FOURCC_OFFSET..DDS_PIXELFORMAT_FOURCC_OFFSET + 4]
            .copy_from_slice(b"DX10");
        normalized[DDS_HEADER_DXT10_MISC_FLAGS2_OFFSET..DDS_HEADER_DXT10_MISC_FLAGS2_OFFSET + 4]
            .fill(0);
        normalized.drain(DDS_HEADER_WITH_DXT10_LEN..DDS_HEADER_WITH_DXT10_LEN + XBOX_EXTENSION_LEN);
        Some(Cow::Owned(normalized))
    }

    fn dds_bytes_for_load<'this>(bytes: &'this [u8]) -> Cow<'this, [u8]> {
        Self::normalized_xbox_dds(bytes).unwrap_or(Cow::Borrowed(bytes))
    }

    fn make_xbox_chunks_from_bytes(
        metadata: &TexMetadata,
        bytes: &[u8],
        options: &ReadOptions,
    ) -> Result<Vec<Chunk<'bytes>>> {
        let mip_sizes = Self::xbox_mip_sizes(metadata, bytes.len())?;
        let chunk_limit = metadata.format.compute_pitch(
            options.mip_chunk_width,
            options.mip_chunk_height,
            CP_FLAGS::CP_FLAGS_NONE,
        )?;

        let chunk_from_mips = |range: Range<usize>| -> Result<Chunk<'bytes>> {
            let offset: usize = mip_sizes[..range.start].iter().sum();
            let len: usize = mip_sizes[range.clone()].iter().sum();
            let mips = range.start.try_into()?..=range.end.saturating_sub(1).try_into()?;
            Ok(Chunk {
                bytes: CompressableBytes::from_owned(
                    bytes[offset..offset + len].to_vec().into(),
                    None,
                ),
                mips: Some(mips),
            })
        ...
        if metadata.is_cubemap() {
            return Ok([chunk_from_mips(0..mip_sizes.len())?].into_iter().collect());
        }
        ...
        loop {
            ...
            if size == 0 || size + mip_size < chunk_limit.slice {
                size += mip_size;
            } else {
                chunks.push(chunk_from_mips(start..stop)?);
                start = stop;
                size = mip_size;
            }
        ...
        if stop < mip_sizes.len() {
            chunks.push(chunk_from_mips(stop..mip_sizes.len())?);
        } else {
            chunks.push(chunk_from_mips(start..stop)?);
        }
    ...
    fn xbox_mip_sizes(metadata: &TexMetadata, data_len: usize) -> Result<Vec<usize>> {
        let mut mip_sizes = Vec::with_capacity(metadata.mip_levels);
        ...
        for _ in 0..metadata.mip_levels {
            ...
            mip_sizes.push(pitch.slice);
            width = usize::max(1, width / 2);
            height = usize::max(1, height / 2);
        }

        let base_total: usize = mip_sizes.iter().sum();
        if base_total == data_len {
            return Ok(mip_sizes);
        }

        let aligned = |align: usize| -> usize {
            mip_sizes
                .iter()
                .map(|&size| size.div_ceil(align) * align)
                .sum()
        };
        let maybe_align = (0..=16)
            .map(|shift| 1usize << shift)
            .find(|&align| aligned(align) == data_len);

        if let Some(align) = maybe_align {
    ...
    where
        In: ?Sized + Source<'bytes>,
    {
        let normalized_xbox = Self::normalized_xbox_dds(stream.as_bytes());
        let bytes = normalized_xbox
            .as_deref()
            .map_or_else(|| Cow::Borrowed(stream.as_bytes()), Cow::Borrowed);
        ...
        let header: Header = DX10 {
            ...
            tile_mode: if options.compression_level() == CompressionLevel::FO4Xbox {
                13
            } else {
                8
            },
        ...
        let chunks = if let Some(normalized_xbox) = normalized_xbox.as_deref() {
            Self::make_xbox_chunks_from_bytes(&meta, &normalized_xbox[148..], options)?
        } else {
            Self::make_chunks(&scratch, options)?
        };
    ...
    where
        In: ?Sized + Source<'bytes>,
    {
        let bytes = Self::dds_bytes_for_load(stream.as_bytes());
        let scratch = ScratchImage::load_dds(&bytes, DDS_FLAGS::DDS_FLAGS_NONE, None, None)?;
        let metadata = scratch.metadata();
        let gnmf = {
...
mod tests {
    ...
    fn dds_bytes_for_load_normalizes_xbox_headers() {
        ...
        bytes[84..88].copy_from_slice(b"XBOX");
        bytes[128..132].copy_from_slice(&98u32.to_le_bytes());
        ...
        bytes[148..152].copy_from_slice(&13u32.to_le_bytes());
        bytes[152..156].copy_from_slice(&256u32.to_le_bytes());
        bytes[156..160].copy_from_slice(&6144u32.to_le_bytes());
        bytes[160..164].copy_from_slice(&13202u32.to_le_bytes());
        bytes[164..168].copy_from_slice(&16u32.to_le_bytes());
        bytes[168..172].copy_from_slice(&262080u32.to_le_bytes());

        let normalized = File::dds_bytes_for_load(&bytes).into_owned();

        assert_eq!(normalized.len(), bytes.len() - 16);
        assert_eq!(&normalized[..4], b"DDS ");
        assert_eq!(&normalized[84..88], b"DX10");
        assert_eq!(
            u32::from_le_bytes(normalized[144..148].try_into().unwrap()),
            0
        );
        assert_eq!(
            u32::from_le_bytes(normalized[148..152].try_into().unwrap()),
            16
        );
        assert_eq!(
            u32::from_le_bytes(normalized[152..156].try_into().unwrap()),
            262080
        );
    }
}
