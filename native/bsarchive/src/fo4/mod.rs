...
pub use self::{
    ...
    file::{
        CapacityError as FileCapacityError, DX10 as DX10Header, File, GNMF as GNMFHeader,
        Header as FileHeader, ReadOptions as FileReadOptions,
    ...
    hashing::{FileHash, Hash, hash_file, hash_file_in_place},
};
...
pub enum Error {
    ...
    #[error(
        "buffer failed to decompress to the expected size... expected {expected} bytes, but got {actual} bytes"
    )]
    ...
    #[error(transparent)]
    LZ4Compress(#[from] block::CompressError),

    #[error(transparent)]
    LZ4Decompress(#[from] block::DecompressError),
...
pub enum CompressionLevel {
    ...
    /// Explicit zlib level override.
    ///
    /// This is used by the project packer to match per-call
    /// compression selection instead of the original preset-only enum.
    Custom(u32),
}
