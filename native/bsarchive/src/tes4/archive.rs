use crate::{
    ...
    tes4::{
        self, Directory, DirectoryHash, DirectoryKey, Error, File, Hash, Result, Version,
        directory::Map as DirectoryMap,
    },
};
...
#[derive(Clone, Copy)]
struct SerializedFileData<'this, 'bytes> {
    file: &'this File<'bytes>,
    embedded_name: Option<&'this BStr>,
}

impl SerializedFileData<'_, '_> {
    fn eq_payload(&self, other: &Self) -> bool {
        self.embedded_name == other.embedded_name
            && self.file.decompressed_len() == other.file.decompressed_len()
            && self.file.as_bytes() == other.file.as_bytes()
    }

    fn serialized_len(&self) -> usize {
        let mut size = self.file.len();
        if let Some(name) = self.embedded_name {
            size += name.len() + 1;
        }
        if self.file.is_compressed() {
    ...
    fn signature(&self) -> (usize, u64) {
        ...
        if let Some(len) = self.file.decompressed_len() {
            hasher.write_u8(1);
        ...
        (self.serialized_len(), hasher.finish())
    }

    fn write_to<Out>(&self, sink: &mut Sink<Out>) -> Result<()>
    where
        Out: ?Sized + Write,
    {
        if let Some(name) = self.embedded_name {
        ...
        sink.write_bytes(self.file.as_bytes())?;
        Ok(())
    }
}
...
impl<'bytes> Archive<'bytes> {
        ...
        Out: ?Sized + Write,
    {
        ...
        let payloads: Vec<_> = directories
            .iter()
            .flat_map(|directory| {
                directory.files.iter().map(|file| SerializedFileData {
                    file: file.this,
                    embedded_name: file.embedded_name.as_ref().map(AsRef::as_ref),
                })
            })
            .collect();
        let payload_plan =
            Self::make_payload_plan(&payloads, offsets.file_data, options.share_data)?;
            ...
            for file in &directory.files {
                Self::write_file_entry(
                    ...
                    *options,
                    file.key,
                    file.this,
                    payload_plan.offsets[payload_index],
        ...
        for &index in &payload_plan.unique_indices {
            payloads[index].write_to(&mut sink)?;
        }
    ...
    fn make_payload_plan(
        payloads: &[SerializedFileData<'_, 'bytes>],
        base_offset: usize,
        share_data: bool,
    ) -> Result<PayloadPlan> {
        ...
        for (index, payload) in payloads.iter().enumerate() {
            ...
            let offset = if let Some(existing) = existing {
                ...
                let offset = next_offset.try_into()?;
                next_offset = next_offset
                    .checked_add(payload.serialized_len())
                    .ok_or(Error::IntegralOverflow)?;
...
mod tests {
    ...
    #[test]
    fn write_with_shared_data_reuses_duplicate_payloads() -> anyhow::Result<()> {
        let payload = b"duplicate payload for tes4";
        let directory: Directory = [
            (
                DirectoryKey::from("first.txt"),
                File::from_decompressed(payload.as_slice()),
            ),
            (
                DirectoryKey::from("second.txt"),
                File::from_decompressed(payload.as_slice()),
            ),
        ]
        .into_iter()
        .collect();
        let archive: Archive = [(ArchiveKey::from("misc"), directory)]
            .into_iter()
            .collect();
        ...
        let plain = {
            ...
            archive.write(
                &mut bytes,
                &ArchiveOptions::builder().version(Version::v104).build(),
            )?;
            bytes
        };
        ...
        let directory = shared_archive
            .get(&ArchiveKey::from("misc"))
            .context("failed to get shared-data directory")?;
        for name in ["first.txt", "second.txt"] {
            let file = directory
                .get(&DirectoryKey::from(name))
                .with_context(|| format!("failed to get shared-data file: {name}"))?;
            assert!(file.is_decompressed());
