...
#[derive(Clone, Copy)]
struct SerializedChunkData<'this, 'bytes> {
    chunk: &'this Chunk<'bytes>,
}

impl SerializedChunkData<'_, '_> {
    fn eq_payload(&self, other: &Self) -> bool {
        self.chunk.decompressed_len() == other.chunk.decompressed_len()
            && self.chunk.as_bytes() == other.chunk.as_bytes()
    }

    fn signature(&self) -> (usize, u64) {
        let mut hasher = DefaultHasher::new();
        if let Some(len) = self.chunk.decompressed_len() {
            hasher.write_u8(1);
            hasher.write_usize(len);
        } else {
            hasher.write_u8(0);
        }
        hasher.write(self.chunk.as_bytes());
        (self.chunk.len(), hasher.finish())
    }
}

impl Offsets {
    #[must_use]
    pub fn new(archive: &Archive, options: Options, data_size: usize) -> Self {
        ...
        let strings_offset = { file_data_offset + data_size };
...
impl OptionsBuilder {
    ...
    #[must_use]
    pub fn share_data(mut self, share_data: bool) -> Self {
        self.0.share_data = share_data;
        self
    }
...
///
/// ```rust
/// use bsarchive_native::fo4::{ArchiveOptions, CompressionFormat, Format, Version};
///
...
impl<'bytes> Archive<'bytes> {
    ...
    where
        Out: ?Sized + Write,
    {
        self.write_impl(stream, options, None)
    }

    pub fn write_in_order<Out>(
        &self,
        stream: &mut Out,
        options: &Options,
        order: &[FileHash],
    ...
    where
        Out: ?Sized + Write,
    {
        self.write_impl(stream, options, Some(order))
    }

    fn ordered_entries<'this>(
        &'this self,
        order: Option<&[FileHash]>,
    ) -> Vec<(&'this Key<'bytes>, &'this File<'bytes>)> {
        ...
        for hash in order {
            if let Some((key, file)) = self.get_key_value(hash) {
    ...
    where
        Out: ?Sized + Write,
    {
        let entries = self.ordered_entries(order);
        let payloads: Vec<_> = entries
            .iter()
            .flat_map(|(_, file)| file.iter())
            .map(|chunk| SerializedChunkData { chunk })
            .collect();
        let payload_plan = self.make_payload_plan(&payloads, *options)?;
        let mut sink = Sink::new(stream);
        let (header, _offsets) = self.make_header(*options, payload_plan.data_size)?;
        ...
        for &index in &payload_plan.unique_indices {
            sink.write_bytes(payloads[index].chunk.as_bytes())?;
        }
    ...
    fn make_header(&self, options: Options, data_size: usize) -> Result<(Header, Offsets)> {
        let offsets = Offsets::new(self, options, data_size);
        Ok((
            Header {
    ...
    fn make_payload_plan(
        &self,
        payloads: &[SerializedChunkData<'_, 'bytes>],
        options: Options,
    ) -> Result<PayloadPlan> {
        let mut next_offset = match options.version {
            Version::v1 | Version::v7 | Version::v8 => constants::HEADER_SIZE_V1,
            Version::v2 => constants::HEADER_SIZE_V2,
            Version::v3 => constants::HEADER_SIZE_V3,
        };
        ...
        let files_len = self.len();
        let chunks_count = payloads.len();
        next_offset += (files_len * usize::from(file_header_size)) + (chunks_count * chunk_size);

        let base_offset = next_offset;
        let mut offsets = Vec::with_capacity(payloads.len());
        let mut unique_indices = Vec::with_capacity(payloads.len());
        let mut seen: HashMap<(usize, u64), Vec<usize>> = HashMap::new();

        for (index, payload) in payloads.iter().enumerate() {
            let signature = payload.signature();
            let existing = if options.share_data {
                seen.get(&signature).and_then(|candidates| {
                    candidates
                        .iter()
                        .copied()
                        .find(|&candidate| payload.eq_payload(&payloads[candidate]))
                })
            } else {
                None
            };

            let offset = if let Some(existing) = existing {
                offsets[existing]
            } else {
                let offset: u64 = next_offset.try_into()?;
                next_offset = next_offset
                    .checked_add(payload.chunk.len())
                    .ok_or(Error::IntegralOverflow)?;
                unique_indices.push(index);
                if options.share_data {
                    seen.entry(signature).or_default().push(index);
                }
                offset
            };
        ...
        Ok(PayloadPlan {
            offsets,
            unique_indices,
            data_size: next_offset - base_offset,
        })
    }
        ...
        Out: ?Sized + Write,
    {
        ...
        for chunk in file {
            Self::write_chunk(sink, header, payload_plan.offsets[*chunk_index], chunk)?;
            *chunk_index += 1;
        }
...
mod tests {
    ...
    #[test]
    fn write_with_shared_data_reuses_duplicate_chunks() -> anyhow::Result<()> {
        let payload = b"duplicate payload for fo4";
        let file_a = File::read(Borrowed(payload.as_slice()), &Default::default())?;
        let file_b = File::read(Borrowed(payload.as_slice()), &Default::default())?;
        let archive: Archive = [
            (ArchiveKey::from("misc/first.txt"), file_a),
            (ArchiveKey::from("misc/second.txt"), file_b),
        ]
        .into_iter()
        .collect();
        ...
        let plain = {
            ...
            archive.write(&mut bytes, &ArchiveOptions::builder().strings(true).build())?;
            bytes
        };
        ...
        let (shared_archive, options) = Archive::read(Borrowed(&shared))?;
        assert!(!options.share_data());
        let write_options: crate::fo4::FileWriteOptions = options.into();
        for name in ["misc/first.txt", "misc/second.txt"] {
            let file = shared_archive
                .get(&ArchiveKey::from(name))
                .with_context(|| format!("failed to get shared-data file: {name}"))?;
            let mut bytes = Vec::new();
