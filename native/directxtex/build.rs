...
fn make_standard_build() -> Build {
    ...
    build
        ...
        .includes([
            ...
            // Precompiled GPU BC shader bytecode, vendored into the tree so
            // source builds don't need the Windows SDK (fxc) to regenerate it.
            "vendored_shaders",
        ]);
    ...
    if cfg!(windows) && tool.is_like_msvc() {
        build.flag("/openmp");
