...
///
/// TREE base records route through the same static path as 3D objects, since FO4's
/// default tree LOD is 3D and folded into `.bto` (per the golden corpus). Billboard-model
/// trees set `is_billboard=true` and build FlatTrunk quads inside the same `.bto`.
...
/// Per-quad tree generator. Dispatches 3D vs billboard by `settings.trees.trees_3d`.
///
...
/// then data_dirs). If absent, returns empty outputs; the driver reports the warning.
...
///
/// Searches `ctx.paths.output_dir` first, then each `data_dirs` entry, for
/// `{world}_billboard_manifest.json`. Returns `None` without logging if no loadable
/// manifest is found; the caller decides whether to warn.
