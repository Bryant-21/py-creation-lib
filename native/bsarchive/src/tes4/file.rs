...
impl CompressionOptionsBuilder {
    ...
    #[must_use]
    pub fn zlib_level(mut self, zlib_level: u32) -> Self {
        self.0.zlib_level = zlib_level;
        self
    }
...
///
/// ```rust
/// use bsarchive_native::tes4::{FileCompressionOptions, Version};
///
...
impl<'bytes> File<'bytes> {
    pub fn compress_into(&self, out: &mut Vec<u8>, options: &CompressionOptions) -> Result<()> {
        if self.is_compressed() {
            ...
            match options.version {
                Version::v103 => self.compress_into_zlib(out, options.zlib_level),
                Version::v104 => match options.compression_codec {
                    CompressionCodec::Normal => self.compress_into_zlib(out, options.zlib_level),
                },
    ...
    fn compress_into_zlib(&self, out: &mut Vec<u8>, zlib_level: u32) -> Result<()> {
        let mut e = ZlibEncoder::new(out, Compression::new(zlib_level));
